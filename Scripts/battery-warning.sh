...
while true; do
	if [ "Discharging" == "$(cat /sys/class/power_supply/BAT0/status)" ]; then
		...
		if ((battery <= 5)); then
			...
			notify-send --icon=$SETTINGS_DIR/Assets/bat5.png --urgency=critical "BATTERY CRITICAL (5%)" "Plug in your computer."
		elif ((battery <= 10)); then
			if [ "VERY LOW" == "$BATTERY_STATUS" ]; then continue; fi
			export BATTERY_STATUS="VERY LOW"
			notify-send --icon=$SETTINGS_DIR/Assets/bat10.png --urgency=critical "BATTERY VERY LOW (10%)" "Please plug in your computer."
		elif ((battery <= 20)); then
			if [ "LOW" == "$BATTERY_STATUS" ]; then continue; fi
			export BATTERY_STATUS="LOW"
			notify-send --icon=$SETTINGS_DIR/Assets/bat20.png --urgency=normal "BATTERY LOW (20%)" "Plug in your computer when possible."
		else
